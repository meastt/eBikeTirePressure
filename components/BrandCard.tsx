  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-card overflow-hidden">
      ...
      <button
        ...
        className="w-full p-6 flex items-center justify-between hover:bg-surface-light transition-colors duration-200 text-left"
        aria-expanded={isExpanded}
        aria-controls={`brand-${brand.slug}-models`}
      >
        <div className="flex items-center gap-4 flex-1">
          {/* Brand avatar */}
          <div className="w-12 h-12 flex items-center justify-center bg-brand-100 text-brand rounded-lg font-heading font-bold text-xl">
            {brand.initial}
          </div>
        ...
        {/* Expand icon */}
        <div className={`text-text-muted transition-transform duration-200 ${isExpanded ? "rotate-180" : ""}`}>
